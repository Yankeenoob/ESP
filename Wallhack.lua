...
-- Function na gagawa ng ESP
local function AddESP(player)
    local function ApplyESP(character)
        if not character then return end
        -- dito ilagay yung mismong ESP logic mo (BillboardGui, Box, Tracer, etc.)
        print("ESP added to:", player.Name)
    end

    -- kapag may existing character na
...
local function InitChecks(Player)
    ...
    PlayerTable.Connections.UpdateChecks = RunService.RenderStepped:Connect(function()
        if Player.Character and Player.Character.Parent and Player.Character:FindFirstChildOfClass("Humanoid") then
            ...
            if Environment.Settings.AliveCheck then
                if Humanoid.Health > 0 then
                    ...
                    -- Instead of hiding ESP, mark as dead (still visible, can draw red)
                    PlayerTable.Checks.Alive = true
            ...
            if Environment.Settings.TeamCheck then
                if Player.Team == LocalPlayer.Team then
                    -- If you want to still show teammates, just mark them differently
                    PlayerTable.Checks.Team = true
                    PlayerTable.IsTeammate = true
                else
                    PlayerTable.Checks.Team = true
                end
            else
...
local Visuals = {
    AddChams = function(Player)
        ...
        -- Clean old if exists
        if PlayerTable.Connections.Chams then
            PlayerTable.Connections.Chams:Disconnect()
            PlayerTable.Connections.Chams = nil
        end
        if PlayerTable.Chams then
            ...
            pcall(function()
                ...
                for _, v in next, PlayerTable.Chams do
                    for i = 1, 6 do
                        ...
                        v["Quad" .. tostring(i)].Visible = false  -- Start hidden
                    end
                end
            end)
        end
        ...
        PlayerTable.Connections.Chams = RunService.RenderStepped:Connect(function()
            pcall(function()
                ...
                if Environment.Visuals.ChamsSettings.Enabled and Environment.Visuals.ChamsSettings.EntireBody ~= OldEntireBody then
                    UpdateRig()
...
AddESP = function(Player)
    ...
    -- Clean old if exists
    if PlayerTable.Connections.ESP then
        PlayerTable.Connections.ESP:Disconnect()
        PlayerTable.Connections.ESP = nil
    end
    if PlayerTable.ESP then
        PlayerTable.ESP:Remove()
        PlayerTable.ESP = nil
    end

    PlayerTable.ESP = Drawingnew("Text")
    PlayerTable.ESP.Visible = false  -- Start hidden

    PlayerTable.Connections.ESP = RunService.RenderStepped:Connect(function()
        pcall(function()
            PlayerTable.ESP.Visible = false

            if Player.Character and Player.Character.Parent and Player.Character:FindFirstChildOfClass("Humanoid") and Player.Character:FindFirstChild("HumanoidRootPart") and Player.Character:FindFirstChild("Head") and Environment.Settings.Enabled then
                ...
                if Environment.Visuals.ESPSettings.Enabled and OnScreen then
                    ...
                    local Parts, Content, Tool = {
                        ...
                        Distance = "[" .. tostring(mathfloor((Player.Character.HumanoidRootPart.Position - (LocalPlayer.Character and LocalPlayer.Character.HumanoidRootPart and LocalPlayer.Character.HumanoidRootPart.Position or Vector3zero)).Magnitude)) .. "]",
    ...
    AddTracer = function(Player)
        ...
        -- Clean old if exists
        if PlayerTable.Connections.Tracer then
            PlayerTable.Connections.Tracer:Disconnect()
            PlayerTable.Connections.Tracer = nil
        end
        if PlayerTable.Tracer then
            PlayerTable.Tracer:Remove()
            PlayerTable.Tracer = nil
        end

        PlayerTable.Tracer = Drawingnew("Line")
        PlayerTable.Tracer.Visible = false  -- Start hidden
        ...
        PlayerTable.Connections.Tracer = RunService.RenderStepped:Connect(function()
            pcall(function()
                ...
                if Player.Character and Player.Character.Parent and Player.Character:FindFirstChildOfClass("Humanoid") and Player.Character:FindFirstChild("HumanoidRootPart") and Player.Character:FindFirstChild("Head") and Environment.Settings.Enabled then
                    ...
                    if Environment.Visuals.TracersSettings.Enabled and OnScreen then
                        ...
                        PlayerTable.Tracer.To = Environment.Visuals.BoxSettings.Type == 1 
                            ...
                            or Vector2new(_2DVector.X, _2DVector.Y - (HeadOffset.Y - LegsOffset.Y) * 0.75)
    ...
    AddBox = function(Player)
        ...
        -- Clean old if exists
        if PlayerTable.Connections.Box then
            PlayerTable.Connections.Box:Disconnect()
            PlayerTable.Connections.Box = nil
        end
        if PlayerTable.Box then
            for _, obj in pairs(PlayerTable.Box) do
...
-- Connect Box ESP to RenderStepped
PlayerTable.Connections.Box = RunService.RenderStepped:Connect(function()
    pcall(function()
        if not PlayerTable.Box then return end

        -- 1️⃣ Hide all drawings first (ghost-proof)
        for _, obj in pairs(PlayerTable.Box) do
        ...
        -- 2️⃣ Ensure character exists & ESP enabled
        if Player.Character 
            ...
            if OnScreen and Environment.Visuals.BoxSettings.Enabled then
                ...
                if PlayerTable.Box.Square then
                    ...
                    for _, line in pairs({PlayerTable.Box.TopLeftLine, PlayerTable.Box.TopRightLine, PlayerTable.Box.BottomLeftLine, PlayerTable.Box.BottomRightLine}) do
                        if line then
                            line.Visible = true
                            line.Thickness = Environment.Visuals.BoxSettings.Thickness
                            line.Transparency = Environment.Visuals.BoxSettings.Transparency
                            line.Color = Environment.Visuals.BoxSettings.Color
                        end
                    end
    ...
    AddHeadDot = function(Player)
        ...
        -- Clean old if exists
        if PlayerTable.Connections.HeadDot then
            PlayerTable.Connections.HeadDot:Disconnect()
            PlayerTable.Connections.HeadDot = nil
        end
        if PlayerTable.HeadDot then
            PlayerTable.HeadDot:Remove()
            PlayerTable.HeadDot = nil
        end

        PlayerTable.HeadDot = Drawingnew("Circle")
        PlayerTable.HeadDot.Visible = false  -- Start hidden

        PlayerTable.Connections.HeadDot = RunService.RenderStepped:Connect(function()
            pcall(function()
                PlayerTable.HeadDot.Visible = false

                if Player.Character and Player.Character.Parent and Player.Character:FindFirstChildOfClass("Humanoid") and Player.Character:FindFirstChild("Head") and Environment.Settings.Enabled then
                    local Vector, OnScreen = WorldToViewportPoint(Player.Character.Head.Position)

                    if Environment.Visuals.HeadDotSettings.Enabled and OnScreen then
    ...
    AddHealthBar = function(Player)
        ...
        -- Clean old if exists
        if PlayerTable.Connections.HealthBar then
            PlayerTable.Connections.HealthBar:Disconnect()
            PlayerTable.Connections.HealthBar = nil
        end
        if PlayerTable.HealthBar then
            ...
            PlayerTable.HealthBar = nil
        end

        PlayerTable.HealthBar = {
            Main = Drawingnew("Square"),
            Outline = Drawingnew("Square")
        }
        PlayerTable.HealthBar.Main.Visible = false  -- Start hidden
        ...
        PlayerTable.Connections.HealthBar = RunService.RenderStepped:Connect(function()
            pcall(function()
                ...
                if Player.Character and Player.Character.Parent and Player.Character:FindFirstChildOfClass("Humanoid") and Player.Character:FindFirstChild("HumanoidRootPart") and Environment.Settings.Enabled then
                    local Vector, OnScreen = WorldToViewportPoint(Player.Character.HumanoidRootPart.Position)

                    if Environment.Visuals.HealthBarSettings.Enabled and OnScreen then
                        local Humanoid = Player.Character:FindFirstChildOfClass("Humanoid")
                        ...
                        PlayerTable.HealthBar.Outline.ZIndex = 1
    ...
    AddCrosshair = function()
        ...
        ServiceConnections.CrosshairConnection = RunService.RenderStepped:Connect(function()
            pcall(function()
                if Environment.Crosshair.Settings.Enabled then
                    -- Left Line
                    Environment.Crosshair.Parts.LeftLine.Visible = true
                    ...
                    -- Right Line
                    Environment.Crosshair.Parts.RightLine.Visible = true
                    ...
                    -- Top Line
                    Environment.Crosshair.Parts.TopLine.Visible = true
                    ...
                    -- Bottom Line
                    Environment.Crosshair.Parts.BottomLine.Visible = true
                    ...
                    -- Center Dot
                    Environment.Crosshair.Parts.CenterDot.Visible = Environment.Crosshair.Settings.CenterDot
        ...
        if PlayerTable then
            ...
            if PlayerTable.Chams then
                for _, v in next, PlayerTable.Chams do
                    for i = 1, 6 do
                        local Quad = v["Quad" .. tostring(i)]
                        if Quad and Quad.Remove then Quad:Remove() end
                    end
                end
            end
            for _, conn in pairs(PlayerTable.Connections) do
                if conn.Connected then conn:Disconnect() end
            end
        end
    end)
...
local function Wrap(Player)
    if not GetPlayerTable(Player) then
        ...
        -- Kondisyon: Huwag i-apply ang ESP kung pareho ang team at TeamCheck ay naka-enable
        if Environment.Settings.TeamCheck and Player.Team == LocalPlayer.Team then
            return -- Huwag ituloy kung teammate
        end
        ...
        PlayerTable.Connections.CharacterAdded = Player.CharacterAdded:Connect(function(Character)
            task.spawn(function()
                repeat task.wait() until Character:FindFirstChild("HumanoidRootPart") and Character:FindFirstChild("Head")

                local PlayerTable = Environment.WrappedPlayers[Player]
                if PlayerTable then
                    PlayerTable.Checks.Alive = true
                    PlayerTable.Checks.Team = true
                end
        ...
        table.insert(ESPConnections[Player], PlayerTable.Connections.CharacterAdded)
    end
end
