...
local function Wrap(Player)
    if not GetPlayerTable(Player) then
        ...
        -- Handle respawns
        PlayerTable.Connections.CharacterAdded = Player.CharacterAdded:Connect(function()
            task.wait(0.5) -- Wait for character to load
            pcall(function()
                AssignRigType(Player)
                Visuals.AddChams(Player) -- Re-create chams
                Visuals.AddESP(Player)   -- Re-create ESP
                Visuals.AddTracer(Player) -- Re-create Tracer
                Visuals.AddBox(Player)   -- Re-create Box
                Visuals.AddHeadDot(Player) -- Re-create HeadDot
                Visuals.AddHealthBar(Player) -- Re-create HealthBar
            end)
        end)
