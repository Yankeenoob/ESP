...
getgenv().ExunysDeveloperESP = {
	DeveloperSettings = {
		...
		UnwrapOnCharacterAbsence = false, -- Changed to false to prevent premature unwrapping
	...
	Settings = {
		Enabled = true,
		PartsOnly = false,
		TeamCheck = false, -- Changed to false to detect all players
		AliveCheck = true,
...
local UpdatingFunctions = {
	ESP = function(Entry, TopTextObject, BottomTextObject)
		...
		if getrenderproperty(TopTextObject, "Visible") then
			...
			setrenderproperty(TopTextObject, "Color", GetColor(Entry.Object, Settings.RainbowColor and CoreFunctions.GetRainbowColor() or Settings.OutlineColor))
...
local UtilityFunctions = {
	InitChecks = function(self, Entry)
		...
		Entry.Connections.UpdateChecks = Connect(__index(RunService, DeveloperSettings.UpdateMode), function()
			...
			if not IsAPlayer and not PartHasCharacter then -- Part
				Checks.Ready = (__index(Player, "Position") - LocalCharacterPosition).Magnitude <= RenderDistance
				return
			end
			...
			if not IsAPlayer then -- NPC
				...
				local IsInDistance = (__index(Player, "Position") - LocalCharacterPosition).Magnitude <= RenderDistance
			...
			-- Player check with retry mechanism
			...
			while not Character and attempts < maxAttempts do
				wait(0.5) -- Wait before retrying
				Character = __index(Player, "Character")
				attempts = attempts + 1
			end
			...
			if Character and IsDescendantOf(Character, Workspace) and Humanoid and Head then
				...
				Checks.Alive = false
				Checks.Team = false
				if DeveloperSettings.UnwrapOnCharacterAbsence and attempts >= maxAttempts then
					self.UnwrapObject(Hash)
				end
				Checks.Ready = false
				print("Player not ready:", Entry.Name, "Alive:", Checks.Alive, "Team:", Checks.Team, "InDistance:", IsInDistance)
				return
			end
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		assert(self, "EXUNYS_ESP

System: > UtilityFunctions.WrapObject - Internal error, unassigned parameter \"self\".")
		...
		spawn(function()
			...
			print("Wrapping player:", Entry.Name)
...
local LoadESP = function()
	for _, Value in next, GetPlayers() do
		...
		print("Wrapping player:", __index(Value, "Name"))
	...
	ServiceConnections.PlayerAdded = Connect(__index(Players, "PlayerAdded"), function(Player)
		...
		for _, Entry in next, WrappedObjects do
			...
			Entry.Connections[__index(Player, "Name").."CharacterAdded"] = Connect(__index(Player, "CharacterAdded"), function(Character)
				for _, _Value in next, Environment.UtilityAssets.WrappedObjects do
					if not _Value.Name == __index(Character, "Name") then
						continue
					end

					UtilityFunctions:UnwrapObject(nil, _Value.Hash)
					UtilityFunctions:WrapObject(GetPlayerFromCharacter(Character))
				end
			end)
		end
	end)
	...
	ServiceConnections.CharacterAdded = Connect(__index(Workspace, "DescendantAdded"), function(Object)
		...
		local WrappedObjects = Environment.UtilityAssets.WrappedObjects
		local Entry = UtilityFunctions:GetObjectEntry(Player)
		if not Entry then
